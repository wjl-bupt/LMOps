...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        print(f"Vllm rollout max_model_len = {max_model_len}")

        # # Calculate max_model_len: consider val_kwargs.max_tokens if it's larger than response_length
        # # This ensures validation can use longer sequences if configured
        # val_max_tokens = getattr(config.val_kwargs, "max_tokens", None)
        # max_response_length = max(
        #     config.response_length,
        #     val_max_tokens if val_max_tokens is not None else config.response_length
        # )
        ...
        repetition_penalty_value = kwargs.get("repetition_penalty", 1.0)
        print(f"[vLLM Rollout] repetition_penalty set to: {repetition_penalty_value}")
