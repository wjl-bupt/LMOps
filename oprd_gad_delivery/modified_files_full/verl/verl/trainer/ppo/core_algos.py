...
def compute_policy_loss_vanilla(
    ...
    if log_prob.dim() == 3 and old_log_prob.dim() == 3:
        ...
        # On-policy (ppo_epochs=1): old_log_prob is detached current log_prob, ratio=1.
        # Use -A * log_prob instead of materializing 3D ratio/exp tensors (saves VRAM on long seqs).
        if ppo_epochs == 1:
            pg_losses = torch.sum(-advantages * log_prob, dim=-1)
            with torch.no_grad():
